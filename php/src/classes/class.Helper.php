...
class Helper {
    ...
    public static function ProcessMentions($mentions) {
        ...
        foreach ($mentions as $mention) {
            ...
            if (Helper::isReplyTo($mention)) {
                ...
                $helper = new Helper();
                $dateandcontent = $helper->getScheduledatDate(
                    $content,
                    $mention
                );
                ...
                if ($schedule_at) {
                    ...
                    $rest_text = Helper::buildRestText(
                        $pre_string,
                        $post_string
                    );
                    ...
                    $reminder = $status->scheduleReminder(
                        $reminder_parameters,
                        $rest_text,
                        $is_reply
                    );
    ...
    public static function doCurlGETRequest($parameters) {
        ...
        if (array_key_exists("api_parameters", $parameters)) {
            if (is_array($parameters["api_parameters"])) {
                ...
                $parameters_http_query = http_build_query(
        ...
        if ($result === false) {
            ...
            $error_status_message =
                "@remindme@toot.re cURL error: " . curl_error($curl_handle);
            $error_data = [
    ...
    public static function getLastSeenMentionId() {
        ...
        if (file_exists(self::getEnvironment()["last_mention_file"])) {
            ...
            if (filesize(self::getEnvironment()["last_mention_file"]) > 0) {
                $mention_file_id = file_get_contents(
                    self::getEnvironment()["last_mention_file"]
                );
                if ($mention_file_id) {
    ...
    function getScheduledatDate($str, $mention) {
        ...
        $omit_words = ["in", "on", "and", "remind", "me"];

        $content = ltrim(
            preg_replace("/(\s+|^)@\S+/", "", strip_tags($time_str))
        );
